...
import { Box, Container, Typography, Button, TextField, FormControlLabel, Switch, Snackbar, Alert, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions } from "@mui/material";
...
function Announcement({ user }) {
    ...
    const handleModify = async (announcementId) => {
        if (!newContent.trim()) {
            alert("公告内容不能为空！");
            return;
        }
        ...
        try {
            const response = await fetch(`/api/announcements/modify`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    id: announcementId,
                        ...
                        <Box display={"flex"} justifyContent={"space-between"} flexDirection={"row"}>
                            <Box item="true" key={id} sx={{ mt: 1, mb: 1 }} flexDirection={"column"}>
                                <Typography justifySelf={"flex-start"} variant="body1" color="black" gutterBottom>
                                    {priority ? <PushPinRoundedIcon color="disabled" sx={{ fontSize: 15, mr: 1 }} /> : <></>}{content}
                                </Typography>
                            ...
                            <Box item="true" justifySelf={"flex-end"} alignContent={"center"} minWidth={128}>
                                <AlertDialog
                                    button={<Button onClick={() => { setModifyOpen(true); }}>修改</Button>}
                                    title={`确定要修改该公告吗？`}
                                    description={<>
                                            ...
                                            <Box display={"flex"} alignItems={"center"}>
                                                <LocalizationProvider dateAdapter={AdapterLuxon}>
                                                    <DateTimePicker
                                                        ...
                                                        ampm={false}
                                                        views={['year', 'month', 'day']}
                                                        defaultValue={valid_until ? DateTime.fromISO(valid_until).setZone("Asia/Shanghai") : null}
                                                        onChange={(newValue) => setNewValidUntil(newValue)}
                                                        slotProps={{ textField: { fullWidth: "true" } }}
                                                    />
                                                </LocalizationProvider>
                                                <FormControlLabel control={<Switch defaultChecked={priority} onChange={(e) => { setStick(e.target.value) }} />} label="置顶公告" labelPlacement="start" />
                                            </Box>
                                        </Box>
                                    </>}
                                    agreeAction={() => { setModifyOpen(false); handleModify(id); }}
                                />
                                <AlertDialog
